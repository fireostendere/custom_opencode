...
## В TUI остаётся вращающаяся строка `Compaction`

В закреплённой версии native engine прерванный компактинг иногда остаётся в истории со статусом `running`. TUI переносит такую запись в конец даже после продолжения работы.

TUI bundle восстанавливает отображение: если после компактинга в этой же сессии уже появился новый шаг assistant или следующий компактинг, старая запись больше не считается активной. Она остаётся на своём месте в истории с пояснением о неполученном итоговом статусе. Позднее настоящее завершение имеет приоритет.

Последний действительно работающий компактинг не снимается по таймеру, очереди сообщений или активности другой сессии. Исправление затрагивает только TUI read model: не изменяет БД, не удаляет историю и не останавливает выполнение. Завершённая строка `Compaction` без вращения — обычная отметка в истории, а не зависание.

## `Session location unavailable` в TUI

Это сообщение возникает при ошибке синхронизации рабочего окружения, не только
при удалённой папке. Наш TUI-плагин повторяет полную проверку через 15 секунд,
не больше 5 раз после исходного сбоя. Во время ожидания показан номер попытки;
после успешной загрузки предупреждение исчезает автоматически. Папка сессии
не меняется, сервер и выполняемые задачи не перезапускаются.

После пяти неудач остаётся штатное окно `Choose directory`. Во время ожидания
можно также выбрать папку через `/move`. Уход из сессии, смена её папки/workspace
или выгрузка плагина отменяют оставшиеся повторы. Медленные проверки не перекрываются.

Проверки: `node scripts/location-recovery-regression.mjs` и
`bash scripts/tui-package-smoke.sh` (изолированный TUI, без запросов к модели).
