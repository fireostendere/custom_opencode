...
`custom_opencode` — переносимый комплект поверх OpenCode V2: web/PWA-интерфейс, Alibaba/Qwen routing, безопасное открытие локальных проектов, server-side control plane, диагностика, self-test и опциональный инженерный RAG через MCP.
...
- [Server control plane](control-plane.md) — risk policy, automatic low-risk permissions, audit и fail-closed boundaries.
...
- `custom_opencode` — UI, OpenCode V2 config, модели, agents, MCP wiring, installer, self-test и lifecycle/control-plane logic;
...
Browser / PWA
    |
    v
custom_opencode web proxy + control plane
    |
    +--> OpenCode V2 backend
    |       |
    |       +--> Build / Plan user modes
    |       +--> native permissions (server-classified before optional auto-reply)
...
Server control plane не доверяет browser или модели право объявить действие безопасным: фактический permission payload перечитывается из OpenCode backend. Неоднозначные, destructive, external и sensitive действия остаются интерактивными.
