...
Пользовательский web UI этого комплекта поддерживает `Build` и `Plan`. В `Plan` session отправляется с native `plan` или `plan-direct`, поэтому модель может создать session-scoped native V2 plan document; созданный документ остаётся видимым в web UI и TUI после переключения обратно в `Build`. `Build` сохраняет write-возможности, а `Plan` остаётся read-only режимом. При работе через custom Runtime V2/V3 дополнительно используются durable task queue, checkpoints и typed handoff.
