...
assert.ok(workspacePanel.includes('side === "right" && x >= width - size'), 'Right overlays must hide the cursor starting at their first occupied cell')
assert.ok(workspacePanel.includes('side === "bottom" && y >= height - size'), 'Bottom overlays must hide the cursor starting at their first occupied cell')
assert.ok(!workspacePanel.includes('<Show when={props.side === "right"}>\n                  <box width={HANDLE} flexShrink={0} />'), 'The right panel must not waste a cell before its scrollbox')
assert.match(workspacePanel, /<scrollbox[\s\S]*?flexGrow=\{1\}[\s\S]*?minWidth=\{0\}/, 'Panel scrollboxes must allow horizontal shrink')
assert.match(workspacePanel, /<box flexDirection="column" width="100%" minWidth=\{0\} paddingX=\{1\} paddingTop=\{1\} paddingBottom=\{1\}/, 'Panel scroll content must fill and shrink within the viewport')
...
assert.match(panelViews, /function WindowRows\(props\)[\s\S]*?<box flexDirection="column" width="100%" minWidth=\{0\} flexShrink=\{0\}>/, 'Limit windows must fit narrow panels')
assert.match(panelViews, /function LimitsView\(\)[\s\S]*?<box flexDirection="column" width="100%" minWidth=\{0\} gap=\{1\} flexShrink=\{0\}>/, 'Limits view must fit narrow panels')
