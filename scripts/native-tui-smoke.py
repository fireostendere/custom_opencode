...
def main() -> int:
    ...
    session = request(
        ...
        "/api/session",
        {
            ...
            "location": {"directory": str(home / "scratch")},
        },
    )
    sid = session["id"]
    ...
    env = {
        ...
        if not k.startswith(("OPENCODE_", "CUSTOM_OPENCODE_", "TOKEN_PLAN_", "GEMINI_", "GOOGLE_"))
    }
    env.update(
        HOME=str(home),
    ...
    process = subprocess.Popen(
        [str(home / ".local/bin/custom-opencode"), "--session", sid],
        cwd=home / "scratch",
        env=env,
        stdin=slave,
        stdout=slave,
    ...
    cases = [
        ...
        ("/addmcp", "MCP name"),
        ("/addskill", "Skill source"),
        ("/addorchestration", "Provider ID"),
        ("/addmcpprofile", "Profile ID"),
        ("/configure", "Configuration"),
        ("/server", "web server"),
        ("/webserver", "Web"),
        ("/models", "Select Model"),
        ("/panel right session", "Сессия"),
        ("/panel right activity", "RIGHT Activity"),
        ("/panel right plan", "Плана пока нет."),
        ("/panel right orchestration", "Оркестрация пока пустая."),
        ("/panel right history", "История"),
        ("/panel right limits", "Лимиты"),
    ]
    try:
        initial = read(5)
        ...
        rows.append(
            {
                "command": "zero inference",
                "ok": final["cost"] == 0
                and all(tokens[k] == 0 for k in ["input", "output", "reasoning"])
                and all(v == 0 for v in tokens["cache"].values()),
            }
        )
    finally:
        process.terminate()
    ...
    errors = re.findall(
        r"Cannot find (?:package|module)|No renderer found|Plugin failed|Configuration response missing",
        text,
    )
    report = {
        "ok": all(row["ok"] for row in rows) and not errors,
        "geometry": "160x40",
        "checks": rows,
        "errors": errors,
    }
    ...
    for row in rows:
        print(("PASS" if row["ok"] else "FAIL") + " native PTY " + row["command"], flush=True)
