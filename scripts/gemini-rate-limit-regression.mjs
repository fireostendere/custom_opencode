...
try {
  ...
  Date.now = () => now
  const retrying = mod.createRetryFetch(
    async (input) => {
      calls += 1
      if (input instanceof Request) await input.text()
      return new Response(calls === 1 ? "Please retry in 1s." : "ok", {
  ...
  const response = await retrying(
    new Request("https://generativelanguage.googleapis.com/v1/test", {
      method: "POST",
      body: "payload",
    }),
  )
  ...
  assert.equal(
    mod.getUsageSnapshot().requestsToday,
    1,
    "the first request after UTC rollover must be counted",
  )

  let limitedCalls = 0
  const terminal = mod.createRetryFetch(
    async () => {
      limitedCalls += 1
  ...
  assert.equal(
    ...
    "Please retry in 1s.",
    "terminal 429 body must remain readable",
  )
  ...
  await assert.rejects(
    retrying("https://generativelanguage.googleapis.com/v1/test", { signal: controller.signal }),
    { name: "AbortError" },
  )
  Date.now = realNow
  ...
  assert.equal(
    globalThis.fetch,
    original,
    "must not install a budget-bypassing global fetch wrapper",
  )
  let event = {
    model: { providerID: "google" },
    error: { status: 429, message: "Please retry in 51s." },
    attempt: 1,
  }
  await hooks.retry(event)
  assert.deepEqual(event.decision, { retry: true, delay: 30000 })
  event = { ...event, attempt: 3 }
  await hooks.retry(event)
  assert.deepEqual(event.decision, { retry: false })
  event = { model: { providerID: "openai" }, error: { status: 429 }, attempt: 1 }
  await hooks.retry(event)
