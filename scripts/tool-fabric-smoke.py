...
async def main():
    ...
    parser.add_argument(
        "--contracts-only",
        action="store_true",
        help="Run host/SDK/stdio contracts; kernel acceptance is a separate required gate",
    )
    ...
    assert "--as=4398046511104" in Fabric.limit_command(
        ["true"], {}, {"addressSpaceBytes": 4_398_046_511_104}
    )
    with tempfile.TemporaryDirectory(prefix="fabric-smoke-") as temp:
        ...
        for mode in ("ok", "sleep", "fail", "flood"):
            fixture["operations"][mode] = recipe(
                [sys.executable, str(engine), mode, "{output}", "{value}"],
                {"value": TEXT},
                ["value"],
                cache=mode == "ok",
            )
        fixture["operations"]["file"] = recipe(
            [sys.executable, str(engine), "ok", "{output}", "{file}"], {"file": FILE}, ["file"]
        )
        fixture["operations"]["flash"] = recipe(
            [sys.executable, str(engine), "ok", "{output}", "{device}"],
            {"device": TEXT},
            ["device"],
            risk="device-write",
        )
        fixture["operations"]["timeout"] = {
            **copy.deepcopy(fixture["operations"]["sleep"]),
            "timeoutSeconds": 1,
        }
        config = {
            "tools": [fixture],
            "permissions": {
                "fixture": {
                    "trustedHost": True,
                    "cache": True,
                    "toolchainDigest": digest("fixture-v1"),
                }
            },
        }
        container_row = {**fixture, "kind": "container", "image": "example@sha256:" + "a" * 64}
        probe = Fabric(str(project), str(base / "container-command"), config)
        argv, _, _ = probe.command(
            container_row, fixture["operations"]["ok"], {"value": "check"}, base / "output"
        )
        assert not any(
            arg.startswith("PATH=") for arg in argv
        ), "host PATH must not replace image PATH"
        await probe.close()
        ...
        operator.write_text(
            json.dumps({"layers": [layer.name], "permissions": config["permissions"]})
        )
        ...
        assert [row["id"] for row in load_config(operator, project)["tools"]] == [
            "fixture",
            "unrelated-domain",
        ]
        ...
        for bad in ({"layers": [layer.name, layer.name]}, {"layers": ["project/policy.json"]}):
            ...
            try:
                ...
                pass
        layer.write_text(
            json.dumps({"tools": [fixture], "permissions": {"fixture": {"trustedHost": True}}})
        )
        ...
        with (
            patch("subprocess.run", side_effect=AssertionError("discovery spawned subprocess")),
            patch(
                "asyncio.create_subprocess_exec",
                side_effect=AssertionError("discovery spawned subprocess"),
            ),
        ):
            fabric = Fabric(str(project), str(base / "state"), config)
            assert len(fabric.rows) == 1
            assert (
                fabric.search("проверить", include_unavailable=True)["results"][0]["id"]
                == "fixture"
            )
            assert fabric.inspect(["fixture"])["tools"][0]["operations"]["flash"][
                "unavailableReason"
            ]
        ...
        async with Client(server) as client:
            ...
            for file in ("../outside", "/etc/passwd", "secret-link"):
                try:
                    ...
                    pass
            assert not (await fabric.run("fixture", "flash", {"device": "probe-1"}, dry_run=True))[
                "allowed"
            ]
            try:
            ...
            with fabric.store.connect() as db:
                path = db.execute(
                    "SELECT file_path FROM artifacts WHERE id=?", (ref["id"],)
                ).fetchone()[0]
            ...
            assert (
                len(flood["result"]["stdout"]) <= 48000 and len(flood["result"]["stderr"]) <= 48000
            )
            ...
            timed = await terminal(
                fabric, await fabric.run("fixture", "timeout", {"value": "timeout"})
            )
            ...
            interrupted = fabric.store.create_task(
                session_id="tool-fabric",
                project_dir=str(project),
                kind="tool-fabric",
                metadata={"toolId": "fixture", "operation": "sleep"},
            )
        ...
        if args.contracts_only:
            print(
                "SCOPE: kernel isolation excluded; run sandbox-preflight.py and full tool-fabric-smoke.py as a separate required gate",
                flush=True,
            )
        else:
            ...
            with socket.socket() as listener:
                ...
                sandbox_config = {
                    "tools": [sandboxed],
                    "permissions": {"sandboxed": {"runtimeRoots": [str(engine.parent)]}},
                }
                ...
                try:
                    ...
                    assert json.loads(result["result"]["stdout"]) == {
                        "outsideReadable": False,
                        "sourceWritable": False,
                        "hostNetworkAccessible": False,
                    }
                    assert (
                        len(result["result"]["artifacts"]) == 1
                    ), "artifact symlink must not be collected"
                finally:
        ...
        params = StdioServerParameters(
            ...
            args=[
                str(ROOT / "app/tool_fabric_mcp.py"),
                "--workspace",
                str(project),
                "--state-dir",
                str(base / "wire"),
        ...
        upstream_params = StdioServerParameters(
            command=sys.executable, args=[str(ROOT / "scripts/fixtures/tool-fabric-upstream.py")]
        )
        ...
        native = {
            **fixture,
            ...
            "kind": "mcp",
            "runtime": {
                "command": [sys.executable, str(ROOT / "scripts/fixtures/tool-fabric-upstream.py")]
            },
            "operations": {
                "echo": {
                    "inputSchema": schema({"value": TEXT}, ["value"]),
                    "upstreamSchemaDigest": digest(upstream_schema),
                    "risk": "read",
                    "network": False,
                    "tool": "echo",
                }
        ...
        os.environ["PYTHONPATH"] = os.pathsep.join(
            [site.getusersitepackages(), os.environ.get("PYTHONPATH", "")]
        )
        native_config = {
            "tools": [native],
            "permissions": {"native": {"trustedHost": True, "environment": ["PYTHONPATH"]}},
        }
        native_fabric = Fabric(str(project), str(base / "native"), native_config)
        async with Client(make_server(native_fabric)):
            done = await terminal(
                native_fabric, await native_fabric.run("native", "echo", {"value": "hello"})
            )
            assert done["state"] == "completed", done
            assert "hello" in done["result"]["stdout"]
            native_fabric.rows["native"]["operations"]["echo"]["upstreamSchemaDigest"] = "changed"
            drift = await terminal(
                native_fabric, await native_fabric.run("native", "echo", {"value": "hello"})
            )
            assert drift["state"] == "failed", drift
            native_fabric.rows["native"]["operations"]["echo"]["upstreamSchemaDigest"] = digest(
                upstream_schema
            )
            bomb = await terminal(
                native_fabric, await native_fabric.run("native", "echo", {"value": "bomb"})
            )
            assert (
                bomb["state"] == "failed"
            ), "oversized upstream JSON must fail before unbounded parsing"

    print(
        "Tool Fabric passed: offline discovery, 5 MCP tools, modern/legacy stdio, real CLI/upstream execution, schema drift, policy, paths, cache, idempotency, limits, cancellation, restart"
    )
