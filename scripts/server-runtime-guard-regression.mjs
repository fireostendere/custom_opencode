...
globalThis.fetch = async (url, init) => {
  ...
  return new Response(JSON.stringify({ command: 'wrapped', cwd: '/repo', shell: '/bin/sh', env: { OPENCODE_SERVER_PASSWORD: 'must-not-be-regranted', OPENCODE_RUNTIME_PLUGIN_TOKEN: 'must-not-be-regranted' } }), {
    status: 200,
...
try {
  ...
  const event = {
    ...
    env: { SAFE_VALUE: 'ok', GEMINI_API_KEY: 'secret', GOOGLE_API_KEY: 'secret-too',
      OPENCODE_SERVER_PASSWORD: 'web-secret', OPENCODE_BACKEND_PASSWORD: 'backend-secret',
      OPENCODE_RUNTIME_PLUGIN_TOKEN: 'internal-secret', OPENCODE_OPENAI_REFRESH: 'oauth-secret',
      OPENCODE_OPENAI_ACCESS: 'access-secret', OPENCODE_ZEN_KEY: 'zen-secret', OPENCODE_GO_KEY: 'go-secret',
  ...
  for (const name of ['OPENCODE_SERVER_PASSWORD', 'OPENCODE_BACKEND_PASSWORD',
    'OPENCODE_RUNTIME_PLUGIN_TOKEN', 'OPENCODE_OPENAI_REFRESH', 'OPENCODE_OPENAI_ACCESS',
    'OPENCODE_ZEN_KEY', 'OPENCODE_GO_KEY']) assert.ok(!Object.hasOwn(event.env, name), `${name} leaked to shell`)
