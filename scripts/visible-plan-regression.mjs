...
try {
  ...
  const ctx = {
    tool: {
      transform: async (callback) => { callback({ add: (tool) => { planTool = tool } }); return registration() },
      hook: async (name, callback) => { hooks[`tool:${name}`] = callback; return registration() },
    },
    session: {
      hook: async (name, callback) => { hooks[`session:${name}`] = callback; return registration() },
    },
  }
  ...
  assert.match(context.system[0].text, /hidden reasoning/)
  await contextHook(context)
  ...
  assert.equal(readFileSync(target, 'utf8'), '# Видимый план\n\n- [x] Проверить текущий поток\n- [>] Подключить публикацию\n- [ ] Запустить проверки\n')
  assert.equal(statSync(target).mode & 0o777, 0o600)

  const nextTurn = {
    ...context,
    system: [],
    messages: [...context.messages, { role: 'assistant', content: [] }, { role: 'user', content: [{ type: 'text', text: 'Продолжай' }] }],
  }
  ...
  const planContext = { ...context, sessionID: 'ses_plan', agent: 'plan', system: [], messages: context.messages }
  await contextHook(planContext)
  assert.match(planContext.system[0].text, /in Plan publish/)
  await beforeHook({ sessionID: planContext.sessionID, messageID: 'msg_4', tool: 'edit' })
  await beforeHook({ sessionID: 'ses_subagent', agent: 'role-builder', messageID: 'msg_5', tool: 'edit' })
  await assert.rejects(
    planTool.execute({ title: 'Bad', todos: [] }, { sessionID: 'ses_bad', agent: 'build', messageID: 'msg_6' }),
    /1-7/,
  )
  await cleanup()
  console.log('Visible plan regression OK: pinned tool, system policy, mutation gate, atomic session plan')
} finally {
