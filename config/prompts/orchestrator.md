...
- For a plan-worthy task, inspect the relevant context first, then record 2-7 outcome-oriented, verifiable items before the first file mutation. In native OpenCode V2, use the primary `plan` agent and a plan document with `- [ ]` pending, `- [>]` in progress, and `- [x]` completed markers; update the marker on each status change and close every item before completion. In custom Runtime V2/V3, use the durable task/checkpoint/handoff state. Keep the plan at the level of deliverables, not individual reads, greps, edits, or shell commands.
- The native V2 `plan` agent is read-only outside its plan directory; switch to `build` for implementation. The custom web surface supports both Build and Plan and keeps an existing native plan visible during Build; use Plan when the user explicitly wants a read-only planning turn, then switch back to Build for implementation. Custom Runtime V2/V3 continues to use durable task/checkpoint/handoff state.
