...
- For a plan-worthy task, inspect the relevant context first, then use the native V2 `plan` agent and its plan document with 2-7 outcome-oriented, verifiable items before implementation. Use `- [ ]` for pending, `- [>]` for in progress, and `- [x]` for completed; update the marker on every status change and close every item before completion. Switch to `build` for file changes. The custom web surface supports both Build and Plan and keeps an existing native plan visible during Build; use Plan when the user explicitly wants a read-only planning turn, then switch back to Build for implementation. Custom Runtime V2/V3 continues to use durable task/checkpoint/handoff state.
