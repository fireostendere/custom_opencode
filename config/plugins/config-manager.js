...
function normalizeRegistry(value) {
  ...
  return {
    ...
    mcpSettings: {
      mode: "auto",
      ...asObject(raw.mcpSettings),
      sessions: asObject(raw.mcpSettings?.sessions),
    },
  }
}
...
function assertCredentialReference(name, value) {
  if (isSensitiveName(name) && !ENV_REF_RE.test(String(value || "").trim()))
    throw new Error(`${name} must use {env:VAR}`)
}
...
function providerDefinition(input) {
  ...
  if (Array.isArray(input.env))
    definition.env = input.env
      .map((item) => String(item))
      .filter((item) => /^[A-Z_][A-Z0-9_]*$/.test(item))
      .slice(0, 16)
  if (input.settings && typeof input.settings === "object")
    definition.settings = structuredClone(input.settings)
  if (input.headers && typeof input.headers === "object")
    definition.headers = structuredClone(input.headers)
...
function mcpDefinition(input) {
  ...
  for (const flag of ["disabled", "codemode"]) {
    if (config[flag] !== undefined && typeof config[flag] !== "boolean")
      throw new Error(`MCP ${flag} must be a boolean`)
  }
  ...
  if (config.type === "remote") {
    ...
    try {
      ...
      throw new Error("remote MCP requires http(s) url")
    }
    ...
    if (
      ...
      config.command.some((item) => typeof item !== "string" || !item.trim())
    ) {
...
function rejectLocalCommandSecrets(command) {
  for (let index = 0; index < command.length; index += 1) {
    ...
    if (flag && isSensitiveName(flag[1]))
      assertCredentialReference(flag[1], flag[2] === undefined ? command[++index] : flag[2])
    else if (flag && /^(?:env|header)$/i.test(flag[1]))
      rejectLocalCredentialArgument(flag[2] === undefined ? command[++index] : flag[2])
...
function mcpProfileDefinition(input) {
  ...
  if (["auto", "all"].includes(id.toLowerCase()))
    throw new Error("Auto and All are reserved system modes")
  if (!Array.isArray(input.mcp)) throw new Error("profile mcp must be an array")
  const risk = input.risk || "normal"
  if (!["normal", "elevated"].includes(risk))
    throw new Error("profile risk must be normal or elevated")
  const list = (value, label) => {
    if (value === undefined) return []
    if (
      !Array.isArray(value) ||
      value.some((x) => typeof x !== "string" || !x.trim() || x.length > 120)
    )
  ...
  return {
    id,
    name: String(input.name || id).slice(0, 120),
    mcp: [...new Set(input.mcp.map((x) => assertID(x, "MCP name")))],
    risk,
    keywords: list(input.keywords, "keywords"),
    agents: list(input.agents, "agents"),
  }
}
...
export default {
  ...
  async setup(ctx) {
    ...
    const receipt = async (sessionID, input, data) => {
      if (typeof input?.requestID === "string")
        await synthetic(
          ctx,
          sessionID,
          `custom.config.receipt:${input.requestID}\n${JSON.stringify(data)}`,
        )
    }
    ...
    const mutationError = (error, rollbackFailures) => {
      ...
      return new Error(
        `${error.message}; rollback failed: ${rollbackFailures.map((item) => item.message || String(item)).join("; ")}`,
      )
    }
    ...
    await ctx.catalog.transform((catalog) => {
      ...
      for (const [key, definition] of Object.entries(registry.models)) {
        ...
        catalog.model.update(providerID, id, (draft) =>
          Object.assign(draft, structuredClone(definition)),
        )
      }
    ...
    const hasDiscovery = typeof ctx.tool?.transform === "function"
    if (hasDiscovery)
      await ctx.tool.transform((tools) =>
        tools.add({
          name: "mcp_discover",
          description:
            "Find MCP tools in the active profile by task keywords or exact tool name. Loads a bounded native-tool working set for the next step, not new permissions.",
          input: {
            type: "object",
          ...
          execute: async (input, context) => ({
            content: JSON.stringify(discovery.discover(context.sessionID, input)),
          }),
        }),
      )
    ...
    await ctx.mcp.transform((draft) => {
      ...
      if (Object.keys(registry.mcpProfiles).length) {
        // Keep native execution/permissions, then defer large schemas per request below.
        // Global native code-mode snapshots cannot isolate different session profiles.
        for (const [name, config] of Object.entries(installed))
          draft.set(name, { ...config, codemode: false })
      }
    })

    await ctx.skill.transform((draft) => {
      for (const definition of Object.values(registry.skills))
    ...
    await ctx.session.hook("context", async (event) => {
      if (event.tools) {
        ...
        // Tool authorization must not mutate user, assistant, or tool data.
    ...
    if (process.env.OPENCODE_MCP_PROFILE_EVIDENCE)
      await ctx.session.hook("http.request", async (event) => {
        ...
        await appendFile(
          process.env.OPENCODE_MCP_PROFILE_EVIDENCE,
          `${JSON.stringify({ time: new Date().toISOString(), sessionID: event.sessionID, agent: event.agent, profile: exposure.get(event.sessionID)?.id, tools })}\n`,
          { mode: 0o600 },
        )
      })

    await ctx.command.transform((commands) => {
      const add = (name, description, handler, help) =>
        commands.add({
          name,
          description,
          execute: ({ sessionID, prompt }) =>
            enqueueMutation(async () => {
              ...
              try {
                ...
                try {
                  await synthetic(ctx, sessionID, `${name}: ${failure.message}\n\n${help}`)
                } catch {}
                throw failure
              }
      ...
      add(
        "addprovider",
        "Add or update a durable managed provider",
        async (input) => {
        ...
        usage(
          "addprovider",
          '/addprovider {"id":"acme","name":"Acme","package":"@opencode-ai/ai/providers/openai-compatible","env":["ACME_API_KEY"],"settings":{"baseURL":"https://llm.example/v1","apiKey":"{env:ACME_API_KEY}"}}',
        ),
      )

      add(
        "addmodel",
        "Add or update a durable managed model",
        async (input) => {
          const { providerID, id, definition } = modelDefinition(input)
          registry = {
            ...registry,
            models: { ...registry.models, [`${providerID}/${id}`]: definition },
          }
        },
        usage(
          "addmodel",
          '/addmodel {"providerID":"acme","id":"coder","modelID":"qwen3-coder","name":"Coder","capabilities":{"tools":true,"input":["text"],"output":["text"]}}',
        ),
      )

      add(
        "addmcp",
        "Add or update a durable managed MCP server",
        async (input) => {
          const { name, config } = mcpDefinition(input)
          const collision = Object.keys(installed).find(
            (id) => id !== name && mcpNamespace(id) === mcpNamespace(name),
          )
          if (collision)
            throw new Error(`MCP name conflicts with '${collision}' after native normalization`)
          const profiles = { ...registry.mcpProfiles }
          if (input.profiles !== undefined) {
            if (
              !Array.isArray(input.profiles) ||
              input.profiles.some((id) => !Object.hasOwn(profiles, id))
            )
              throw new Error("profiles must contain existing profile IDs")
            for (const [id, profile] of Object.entries(profiles))
              profiles[id] = {
                ...profile,
                mcp: [
                  ...profile.mcp.filter((item) => item !== name),
                  ...(input.profiles.includes(id) ? [name] : []),
                ],
              }
          }
          registry = {
            ...registry,
            mcp: { ...registry.mcp, [name]: config },
            mcpProfiles: profiles,
          }
        },
        usage(
          "addmcp",
          '/addmcp {"name":"docs","config":{"type":"remote","url":"https://mcp.example.com"}}',
        ),
      )

      add(
        "addmcpprofile",
        "Add or update a durable MCP profile",
        async (input) => {
          const profile = mcpProfileDefinition(input)
          registry = {
            ...registry,
            mcpProfiles: { ...registry.mcpProfiles, [profile.id]: profile },
          }
        },
      ...
      add(
        "mcp-profile",
        "Select MCP profile: Auto, All or a registered profile",
        async (input, sessionID) => {
          const mode = String(input.mode || "auto")
          if (!["auto", "all"].includes(mode) && !Object.hasOwn(registry.mcpProfiles, mode))
            throw new Error(`Unknown MCP profile: ${mode}`)
          const settings = registry.mcpSettings
          if (input.scope === "fallback") {
            if (mode === "auto" || registry.mcpProfiles[mode]?.risk === "elevated")
              throw new Error("Auto fallback must be All or a normal profile")
            registry = { ...registry, mcpSettings: { ...settings, fallback: mode } }
          } else if (input.scope === "default")
            registry = { ...registry, mcpSettings: { ...settings, mode } }
          else
            registry = {
              ...registry,
              mcpSettings: {
                ...
                sessions: {
                  ...settings.sessions,
                  [input.agent ? `${sessionID}/${assertID(input.agent, "agent")}` : sessionID]:
                    mode,
                },
              },
            }
      ...
      add(
        "addskill",
        "Add or update a durable managed skill",
        async (input) => {
        ...
        usage(
          "addskill",
          '/addskill {"id":"review","name":"Review","description":"Review current changes","content":"Review the current changes for correctness and regressions."}',
        ),
      )

      add(
        "addorchestration",
        "Add a model-picker orchestration alias with a managed system policy",
        async (input) => {
          const { key, definition } = orchestrationDefinition(input)
          registry = {
            ...registry,
            orchestrations: { ...registry.orchestrations, [key]: definition },
          }
        },
        usage(
          "addorchestration",
      ...
      commands.add({
        ...
        execute: ({ sessionID, prompt }) =>
          enqueueMutation(async () => {
            ...
            await ctx.mcp.list?.()
            if (input.requestID)
              return receipt(sessionID, input, {
                registry,
                installed,
                mode: await modeFor(sessionID),
                exposure: exposure.get(sessionID) || null,
              })
            await synthetic(
              ctx,
              sessionID,
      ...
      commands.add({
        name: "remove-managed",
        description:
          'Remove a managed item: /remove-managed {"type":"models","id":"provider/model"}',
        execute: ({ sessionID, prompt }) =>
          enqueueMutation(async () => {
            const previous = registry
            try {
              ...
              if (
                !["providers", "models", "mcp", "skills", "orchestrations", "mcpProfiles"].includes(
                  type,
                )
              )
                throw new Error(
                  "type must be providers, models, mcp, skills, orchestrations or mcpProfiles",
                )
              if (!Object.prototype.hasOwnProperty.call(registry[type], id))
                throw new Error("managed item not found")
              ...
              try {
                await synthetic(ctx, sessionID, `remove-managed: ${failure.message}`)
              } catch {}
              throw failure
            }
            try {
              await synthetic(ctx, sessionID, "remove-managed: removed")
            } catch {}
          }),
      })
    })
...
if (process.env.OPENCODE_CONFIG_MANAGER_SELF_CHECK) {
  ...
  let rejected = false
  try {
    providerDefinition({ id: "bad", settings: { apiKey: "literal-secret" } })
  } catch {
    rejected = true
  }
  if (!rejected) throw new Error("inline secret self-check failed")
  const camelProvider = providerDefinition({
    id: "camel-provider",
    settings: { privateKey: "{env:PRIVATE_KEY}" },
  })
  const camelModel = modelDefinition({
    providerID: "acme",
    id: "camel-model",
    settings: { clientSecret: "{env:CLIENT_SECRET}" },
  })
  if (camelProvider.id !== "camel-provider" || camelModel.id !== "camel-model")
    throw new Error("camel inline reference self-check failed")
  for (const [definition, input] of [
    [providerDefinition, { id: "bad-private", settings: { privateKey: "literal" } }],
    [
      modelDefinition,
      { providerID: "acme", id: "bad-client", settings: { clientSecret: "literal" } },
    ],
  ]) {
    rejected = false
    try {
  ...
  const remote = mcpDefinition({
    ...
    config: {
      type: "remote",
      url: "https://mcp.example.com?api_key={env:DOCS_TOKEN}&key={env:DOCS_KEY}",
    },
  })
  if (remote.config.type !== "remote") throw new Error("remote MCP self-check failed")
  const camelURL = mcpDefinition({
    name: "camel-url",
    config: {
      type: "remote",
      url: "https://mcp.example.com?clientSecret={env:CLIENT_SECRET}&bearerToken={env:BEARER_TOKEN}",
    },
  })
  ...
  try {
    mcpDefinition({
      name: "bad-url",
      config: { type: "remote", url: "https://user:pass@mcp.example.com?clientSecret=literal" },
    })
  } catch {
    rejected = true
  }
  if (!rejected) throw new Error("remote MCP secret self-check failed")
  const local = mcpDefinition({
    name: "local",
    config: {
      type: "local",
      command: [
  ...
  for (const name of ["clientSecret", "bearerToken", "refreshToken", "privateKey", "xApiKey"]) {
    ...
    rejected = false
    try {
      assertCredentialReference(name, "literal")
    } catch {
      rejected = true
    }
  ...
  for (const command of [
    ...
    ["tool", "Authorization: literal"],
  ]) {
    rejected = false
    try {
      mcpDefinition({ name: "bad-local", config: { type: "local", command } })
    } catch {
      rejected = true
    }
  ...
  if (orchestration.key !== "acme/coder-orchestrated")
    throw new Error("orchestration self-check failed")
