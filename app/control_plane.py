#!/usr/bin/env python3
"""Server-side control-plane primitives for custom_opencode.

The first control-plane slice intentionally stays deterministic: permission
requests are classified locally, only low-risk decisions are auto-approved,
and every automatic decision is auditable. Unknown or ambiguous actions remain
interactive instead of trusting an LLM to widen its own permissions.
"""
    ...
    if isinstance(raw, list):
        ...
        for item in raw:
            if isinstance(item, str):
                ...
                for key in ("path", "command", "resource", "value"):
                    value = item.get(key)
    ...
    *,
    ...
    if action in WRITE_ACTIONS:
        ...
        if any(_looks_sensitive(value) for value in resources):
            ...
            return decision
        if selected in {"workspace", "autonomous"} and all(
            _path_within_workspace(value, workspace) for value in resources
        ):
            decision.update(effect="allow", auto=True, reply="once", risk="R2", reason="workspace-local file mutation")
        else:
            decision.update(risk="R2", reason="file mutation is outside the selected automatic policy")
        return decision
...
def _state_dir() -> Path:
    configured = os.environ.get("CUSTOM_OPENCODE_STATE_DIR")
    return Path(configured).expanduser() if configured else Path.home() / ".local/state/custom-opencode"
...
def audit_decision(
    ...
    *,
    ...
    try:
        ...
        # Audit I/O must never make the permission path fail open or fail closed.
        pass
...
def snapshot() -> dict[str, Any]:
    ...
    return {
        ...
        "permissionPolicy": {
            ...
            "automatic": {
                "readOnly": True,
                "workspaceWrites": preset in {"workspace", "autonomous"},
                "workspaceChecks": preset in {"workspace", "autonomous"},
                "networkFetch": preset == "autonomous",
            },
            "alwaysInteractive": [
                "compound/unknown shell", "external directory", "subagent/task escalation",
                "RAG ingest", "sensitive files", "destructive or external side effects",
            ],
        },
        "modelProfiles": {
            "ordinary": {"agent": "build-direct", "planAgent": "plan-direct", "orchestrated": False},
            "qwen3.8-max-orchestrated": {"agent": "build", "planAgent": "plan", "orchestrated": True, "worker": "fast-reader"},
        },
        "resourcePolicy": {
            "localProvider": os.environ.get("OPENCODE_LOCAL_PROVIDER", "ollama"),
            "localAutoStart": os.environ.get("OPENCODE_LOCAL_AUTO_START", "0") not in {"0", "false", "no"},
        },
    }
