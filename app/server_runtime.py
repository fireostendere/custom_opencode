...
CONTEXT = ContextService(STORE, INDEXER)
VERIFY = VerificationPipeline(ARTIFACTS)
...
LAST_INDEX_AT: dict[str, float] = {}
LAST_PRUNE_AT = 0.0
MIGRATION_DONE = False
PLAN_DIRECTORY = Path(
    os.environ.get("OPENCODE_PLAN_DIRECTORY") or (Path.home() / ".opencode" / "plan")
).expanduser()
PLAN_MAX_BYTES = 1_000_000
PLAN_MAX_ITEMS = 500
ACTIVE_STATES = {"submitted", "running", "waiting_permission", "verifying", "recovering"}
QUEUE_STATES = {"queued", "blocked", "paused"}
PROFILE_IDS = {
    "direct",
    "fast",
    "build",
...
def _model_ref(session: dict[str, Any]) -> str | None:
    ...
    if isinstance(model, dict):
        ...
        return (
            f"{provider}/{ident}" + (f"#{variant}" if variant else "")
...
def _mode(session: dict[str, Any]) -> str:
    return "plan" if "plan" in str(session.get("agent") or "build") else "build"


def _catalog(features: Any, directory: str) -> list[dict[str, Any]]:
    key = hashlib.sha256(directory.encode()).hexdigest()
    cached = STORE.cache_get("model-catalog", key)
    if isinstance(cached, list):
        return [item for item in cached if isinstance(item, dict)]
    try:
        value = features._data(
            features._backend_request_json(
                "GET", features._workspace_target("/api/model", directory), timeout=12.0
            )
        )
        rows = [item for item in value if isinstance(item, dict)] if isinstance(value, list) else []
    except Exception:
        rows = []
    STORE.cache_set("model-catalog", key, rows, ttl_seconds=30)
    return rows


def capability_snapshot(features: Any, directory: str) -> dict[str, Any]:
    REGISTRY.refresh(_catalog(features, directory))
    return REGISTRY.snapshot(stats_getter=STORE.model_stats)


def resource_snapshot() -> dict[str, Any]:
    return SCHEDULER.snapshot(REGISTRY.profiles())
...
def _public(task: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "projectDir": task.get("project_dir"),
        "kind": task.get("kind"),
...
def queue_snapshot(session_id: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "states": _counts(tasks),
        "runtime": "sqlite-v2",
    }


def _usage_totals(features: Any, session_id: str) -> dict[str, Any]:
    totals = {"input": 0, "output": 0, "cacheRead": 0, "cacheWrite": 0, "reasoning": 0, "cost": 0.0}
    rows = []
    cursor = None
    seen_cursors = set()
    try:
        for _ in range(1000):
            path = f"/api/session/{quote(session_id, safe='')}/message?limit=200"
            path += "&cursor=" + quote(cursor, safe="") if cursor else "&order=asc"
            raw = features._backend_request_json("GET", path, timeout=12.0)
            page = features._data(raw)
            if not isinstance(page, list):
                raise ValueError("malformed native message page")
            rows.extend(page)
            cursor = (raw.get("cursor") or {}).get("next") if isinstance(raw, dict) else None
            if not cursor:
                break
            cursor = str(cursor)
            if cursor in seen_cursors:
                raise ValueError("native message cursor repeated")
            seen_cursors.add(cursor)
        else:
            raise ValueError("native usage history exceeds page safety limit")
    ...
    for row in rows:
        blob = (
            row.get("info") if isinstance(row, dict) and isinstance(row.get("info"), dict) else row
        )
        if not isinstance(blob, dict):
            continue
        ident = blob.get("id") or row.get("id")
        ...
        for key in ("input", "output", "reasoning"):
            if isinstance(tokens.get(key), (int, float)):
                totals[key] += max(0, int(tokens[key]))
        for src, dst in (("read", "cacheRead"), ("write", "cacheWrite")):
            if isinstance(cache.get(src), (int, float)):
                totals[dst] += max(0, int(cache[src]))
        if isinstance(blob.get("cost"), (int, float)):
            totals["cost"] += max(0.0, float(blob["cost"]))
        digest.append(json.dumps(row, ensure_ascii=False, sort_keys=True, default=str)[-4000:])
    totals.update(
        signature=(
            hashlib.sha256("\n".join(digest[-20:]).encode()).hexdigest()[:20] if digest else ""
        ),
        available=True,
    )
    return totals
...
def _last_assistant_text(features: Any, session_id: str) -> str:
    try:
        value = features._data(
            features._backend_request_json(
                "GET", f"/api/session/{quote(session_id,safe='')}/message?limit=80", timeout=10.0
            )
        )
    except Exception:
        return ""
    for row in reversed(value if isinstance(value, list) else []):
        ...
        parts = (
            ...
            else row.get("content") if isinstance(row.get("content"), list) else []
        )
        texts = [
...
def enqueue_prompt(features: Any, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    metadata = {
        "selectedModelAtCreate": selected,
        "modeAtCreate": _mode(session),
    ...
    task = STORE.create_task(
        ...
        kind=str(payload.get("kind") or "prompt")[:80],
        metadata=metadata,
        baseline=git_snapshot(directory),
        route=decision.as_dict(),
    )
    STORE.checkpoint(
        task["id"],
        "queued",
        summary="Task persisted in server queue",
        data={"route": decision.as_dict(), "baseline": task.get("baseline")},
    )
    return {
        "ok": True,
        "item": _public(task),
        "task": _public(task),
        "count": queue_snapshot(sid)["count"],
    }
...
def delete_queue_item(session_id: str, item_id: str) -> dict[str, Any]:
    ...
    if not task or task.get("session_id") != session_id:
        return {"ok": False, "count": queue_snapshot(session_id)["count"]}
    if task.get("state") not in QUEUE_STATES:
        return {
            "ok": False,
            "count": queue_snapshot(session_id)["count"],
            "error": "task is not queued",
        }
    STORE.transition(item_id, "cancelled", event="task.cancelled", data={"source": "queue-delete"})
    return {"ok": True, "count": queue_snapshot(session_id)["count"]}


def reorder_queue(session_id: str, ids: list[str]) -> dict[str, Any]:
    STORE.reorder(session_id, ids)
    return {"ok": True, "count": queue_snapshot(session_id)["count"]}


def _switch_session(features: Any, task: dict[str, Any]) -> dict[str, Any]:
    session = features._session_info(task["session_id"])
    metadata = task.get("metadata") if isinstance(task.get("metadata"), dict) else {}
    ...
    if (
        ...
        if sep and provider and ident:
            ...
            if has_variant and variant:
                pinned["variant"] = variant
            features._backend_request_json(
                "POST",
                f"/api/session/{quote(task['session_id'],safe='')}/model",
                {"model": pinned},
                timeout=12.0,
            )
    ...
    if agent and agent != str(session.get("agent") or ""):
        try:
            features._backend_request_json(
                "POST",
                f"/api/session/{quote(task['session_id'],safe='')}/agent",
                {"agent": agent},
                timeout=12.0,
            )
        except Exception:
...
def dispatch_immediate(
    features: Any, *, session_id: str, text: str, files: list[Any], profile: str = "direct"
) -> dict[str, Any]:
    created = enqueue_prompt(
        features,
        {
            "sessionID": session_id,
            "text": text,
...
def _dispatch_task(features: Any, task: dict[str, Any]) -> Any:
    ...
    try:
        route = _switch_session(features, task)
        STORE.update_task(
            task["id"],
            metadata_patch={
                "usageBaseline": _usage_totals(features, task["session_id"]),
                "usageBaselineAt": now_ms(),
            },
        )
        ...
        STORE.checkpoint(
            task["id"],
            "dispatched",
            summary="Prompt accepted for dispatch",
            data={"route": route, "repo": git_snapshot(task["project_dir"])},
        )
        result = features._send_backend_prompt(
            task["session_id"], str(task.get("text") or ""), list(task.get("files") or [])
        )
    except Exception as exc:
        STORE.transition(
            task["id"],
        ...
        STORE.add_usage(
            task_id=task["id"],
            model_ref=(task.get("route") or {}).get("selectedModel"),
            stage=_usage_stage(task),
            latency_ms=int((time.monotonic() - started) * 1000),
            success=False,
        )
        raise
    STORE.update_task(task["id"], metadata_patch={"dispatchAcceptedAt": now_ms()})
    return result
...
def _usage_stage(task: dict[str, Any]) -> str:
    ...
    if kind == "research":
        return "research"
    if kind == "verification-fix":
        return "implementation"
    return (
        "planning"
        if "plan" in str((task.get("metadata") or {}).get("modeAtCreate"))
        else "implementation" if kind == "prompt" else kind
    )
...
def _capture_usage(features: Any, task: dict[str, Any], success: bool) -> None:
    task = STORE.get_task(task["id"]) or task
    baseline = (
        (task.get("metadata") or {}).get("usageBaseline")
        if isinstance(task.get("metadata"), dict)
        else {}
    )
    current = _usage_totals(features, task["session_id"])
    if current.get("available") is False or (baseline or {}).get("available") is False:
        STORE.event(
            kind="usage.unavailable",
            task_id=task["id"],
            session_id=task["session_id"],
            data={"error": current.get("error") or (baseline or {}).get("error")},
        )
        return

    def delta(name: str) -> float:
        try:
            return max(0.0, float(current.get(name, 0)) - float((baseline or {}).get(name, 0) or 0))
    ...
    STORE.add_usage(
        task_id=task["id"],
        model_ref=route.get("selectedModel"),
        stage=_usage_stage(task),
        input_tokens=int(delta("input")),
        output_tokens=int(delta("output")),
        cache_read_tokens=int(delta("cacheRead")),
        cache_write_tokens=int(delta("cacheWrite")),
        cost=delta("cost"),
        latency_ms=max(
            0, now_ms() - int(task.get("started_at") or task.get("created_at") or now_ms())
        ),
        success=success,
        event_key=f"task:{task['id']}:dispatch:{task.get('dispatch_attempts',0)}",
    )
...
def _tool_signatures(features: Any, sid: str) -> list[str]:
    try:
        value = features._data(
            features._backend_request_json(
                "GET", f"/api/session/{quote(sid,safe='')}/message?limit=60", timeout=8.0
            )
        )
    except Exception:
        return []
    return re.findall(
        r'"(?:tool|toolName|name)"\s*:\s*"([A-Za-z0-9_.:-]+)"',
...
def _permission_pending(features: Any, task: dict[str, Any]) -> bool:
    try:
        rows = features._permission_requests(task["project_dir"])
    except Exception:
        return False
    return any(
        str(item.get("sessionID") or item.get("sessionId") or "") == task["session_id"]
        for item in rows
        if isinstance(item, dict)
    )
...
def _monitor_progress(features: Any, task: dict[str, Any]) -> None:
    ...
    repeats = repeats + 1 if signature and signature == old else 0
    if signature != old:
        STORE.checkpoint(
            task["id"],
            "progress",
    ...
    owner = str(metadata.get("ownershipRoot") or task["project_dir"])
    ...
    if loop and not metadata.get("loopReported"):
        ...
        STORE.update_task(task["id"], metadata_patch={"loopReported": True})
    if conflicts and not metadata.get("conflictReported"):
        STORE.event(
            kind="patch.conflict",
            task_id=task["id"],
            session_id=task["session_id"],
        ...
        STORE.update_task(task["id"], metadata_patch={"conflictReported": True})
    ...
    if repeats >= threshold and not metadata.get("stuckReported"):
        ...
        STORE.update_task(task["id"], metadata_patch={"stuckReported": True})
        if os.environ.get("OPENCODE_STUCK_ACTION", "warn").strip().lower() == "interrupt":
            try:
                _interrupt(features, task["session_id"])
                STORE.transition(
                    task["id"], "paused", event="agent.stuck_paused", data={"repeats": repeats}
                )
            except Exception:
                pass
...
def _finish_async(features: Any, task: dict[str, Any]) -> None:
    with VERIFY_LOCK:
        if task["id"] in VERIFYING:
            return
        VERIFYING.add(task["id"])
    threading.Thread(
        target=_verify_finish,
        args=(features, task["id"]),
        name=f"verify-{task['id'][:10]}",
        daemon=True,
    ).start()


def _schedule_review(features: Any, task: dict, review: dict, verification: dict) -> dict:
    from execution_ledger import ExecutionLedger

    root_id = str((task.get("metadata") or {}).get("rootTaskID") or task["id"])
    snapshot = git_snapshot(task["project_dir"])
    change_id = hashlib.sha256(
        json.dumps(snapshot, sort_keys=True, default=str).encode()
    ).hexdigest()
    owner = (
        "orchestrator"
        if task.get("profile") in {"architect", "sol-orchestrated", "critical"}
        else "server"
    )
    ledger = ExecutionLedger(STORE)
    if not ledger.claim_review(root_id, change_id, owner):
        return {"scheduled": False, "reason": "change already assigned to its review owner"}
    STORE.event(
        kind="review.owner",
        task_id=task["id"],
        session_id=task["session_id"],
        data={"rootID": root_id, "changeID": change_id, "owner": owner},
    )
    if owner == "orchestrator":
        return {"scheduled": False, "reason": "native orchestrator owns this review"}
    body = {
        "location": {"directory": task["project_dir"]},
        "title": "Independent read-only review",
        "agent": "plan-direct",
    }
    created = features._data(
        features._backend_request_json("POST", "/api/session", body, timeout=20.0)
    )
    if not isinstance(created, dict) or not created.get("id"):
        raise RuntimeError(
            "Read-only reviewer session creation failed; review claim retained for reconciliation"
        )
    baseline = (task.get("baseline") or {}).get("head") or "HEAD"
    if not re.fullmatch(r"[a-f0-9]{40,64}|HEAD", baseline):
    ...
    evidence = json.dumps(verification, ensure_ascii=False, default=str)[:4000]
    text = (
        "Independently review this bounded change. Do not edit or run commands. Use read-only file tools to verify omitted/untracked files. Return severity, file/line, evidence and missing tests.\n"
        + "Requirements:\n"
        + str(task.get("text") or "")[:4000]
        + "\nChanged files:\n"
        + "\n".join(snapshot.get("changed") or [])[:4000]
        + "\nVerification evidence:\n"
        + evidence
        + "\nDiff excerpt (read changed files for omitted sections):\n"
        + (diff.stdout or "")[:16000]
    )
    review_task = STORE.create_task(
        ...
        files=[],
        profile="review",
        priority=int(task.get("priority") or 0) - 5,
        kind="review",
        metadata={
            "rootTaskID": root_id,
            "reviewOf": task["id"],
        ...
        baseline=task.get("baseline") or snapshot,
    )
    with STORE.transaction() as db:
        db.execute(
            "UPDATE execution_review_claims SET task_id=? WHERE root_id=? AND change_id=?",
            (review_task["id"], root_id, change_id),
        )
    return {"scheduled": True, "taskID": review_task["id"], "sessionID": created["id"]}
...
def _verify_finish(features: Any, task_id: str) -> None:
    try:
        ...
        verification = (
            ...
            if task.get("kind") in {"research", "review", "aggregate"}
            else VERIFY.run(task=task)
        )
        STORE.update_task(task_id, verification=verification)
        STORE.checkpoint(
            task_id,
            "verification",
            summary="Verification pipeline finished",
            data={"ok": verification.get("ok"), "results": verification.get("results", [])},
        )
        task = STORE.get_task(task_id) or task
        actionable = (
            verification.get("actionableFailures") if isinstance(verification, dict) else []
        )
        environment = (
            verification.get("environmentFailures") if isinstance(verification, dict) else []
        )
        _capture_usage(features, task, not bool(actionable))
        review = review_decision(
            task["project_dir"],
            task.get("baseline") if isinstance(task.get("baseline"), dict) else None,
        )
        STORE.event(
        ...
        if actionable:
            STORE.transition(
                task_id,
                "needs_attention",
                event="verification.code_failure",
                data={"failures": actionable},
            )
            STORE.checkpoint(
                task_id,
                "needs_attention",
                summary="Code verification failed; only actionable failures are eligible for repair",
                data={"failures": actionable},
            )
            if (
                os.environ.get("OPENCODE_VERIFY_AUTOFIX", "1").strip().lower()
                not in {"0", "false", "off", "no"}
                and task.get("kind") != "verification-fix"
            ):
                failure = "\n\n".join(str(item.get("failureSummary") or "") for item in actionable)[
                    :12000
                ]
                STORE.create_task(
                    session_id=task["session_id"],
                    project_dir=task["project_dir"],
                    text="Verification found code failures after the previous task. Fix only these failures, preserve the intended change, then finish.\n\n"
                    + failure,
                    files=[],
                    profile=str(task.get("profile") or "build"),
                    priority=int(task.get("priority") or 0) + 5,
                    kind="verification-fix",
                    metadata={
                        "rootTaskID": (task.get("metadata") or {}).get("rootTaskID") or task_id,
                        "repairOf": task_id,
                        "modeAtCreate": "build",
                        "usageBaseline": _usage_totals(features, task["session_id"]),
                        "sandbox": "repo-write",
                    },
                    baseline=git_snapshot(task["project_dir"]),
                )
        else:
            STORE.transition(
                task_id,
                ...
                data={"verificationWarnings": environment},
            )
            STORE.checkpoint(
                task_id,
                "completed",
                summary="Task completed and durable state committed",
                data={"repo": git_snapshot(task["project_dir"]), "verification": verification},
            )
            if task.get("kind") == "research":
                target = (
                    (task.get("metadata") or {}).get("mailboxTo")
                    if isinstance(task.get("metadata"), dict)
                    else None
                )
                if target:
                    STORE.mailbox_send(
                        ...
                        payload={
                            "text": _last_assistant_text(features, task["session_id"])[:14000],
                            "route": task.get("route"),
                        },
                    )
            profile = REGISTRY.profiles().get(
                str(task.get("profile")), REGISTRY.profiles()["direct"]
            )
            if (
                task.get("kind") in {"prompt", "verification-fix"}
                and profile.get("autoReview")
                and review.get("needed")
                and os.environ.get("OPENCODE_AUTO_REVIEW", "smart").strip().lower()
                in {"smart", "enqueue", "1", "true"}
            ):
                _schedule_review(features, task, review, verification)
    except Exception as exc:
        try:
            STORE.transition(
                task_id,
                "failed",
                event="task.finalize_failed",
                data={},
                error=f"{type(exc).__name__}: {exc}",
            )
        except Exception:
...
def _monitor_active(features: Any, statuses: dict[str, Any]) -> None:
    ...
    for task in STORE.list_tasks(states=ACTIVE_STATES, limit=500):
        ...
        if state == "recovering":
            STORE.transition(
                task["id"],
                "running" if busy else "paused",
                event="task.recovered_attached" if busy else "task.recovery_paused",
                data={},
            )
            continue
        if state == "verifying":
        ...
        if pending and state != "waiting_permission":
            STORE.transition(
                task["id"], "waiting_permission", event="task.waiting_permission", data={}
            )
            STORE.checkpoint(
                task["id"],
                "waiting_permission",
                summary="Agent is waiting for a permission decision",
                data={},
            )
            continue
        if state == "waiting_permission" and not pending:
            STORE.transition(
                task["id"],
                "running" if busy else "submitted",
                event="task.permission_resolved",
                data={},
            )
            state = "running" if busy else "submitted"
        if busy:
            if state == "submitted":
        ...
        accepted = int(metadata.get("dispatchAcceptedAt") or 0)
        baseline_usage = (
            metadata.get("usageBaseline") if isinstance(metadata.get("usageBaseline"), dict) else {}
        )
        progressed = bool(metadata.get("backendObservedBusy")) or _usage_totals(
        ...
        with features.STATE_LOCK:
            ...
            for sid, rows in list(queues.items()):
                if not isinstance(rows, list):
                    continue
                try:
                    directory = features._session_directory(str(sid))
                except Exception:
                    continue
                for row in rows:
                    if not isinstance(row, dict):
                    ...
                    if legacy and STORE.get_task(legacy):
                        continue
                    STORE.create_task(
                        session_id=str(sid),
                        project_dir=directory,
                        ...
                        profile=_profile_id(row.get("profile")),
                        kind="prompt",
                        metadata={
                            "migratedFrom": "web-features-v1",
                            "modeAtCreate": "build",
                            "usageBaseline": _usage_totals(features, str(sid)),
                        },
                        baseline=git_snapshot(directory),
                        task_id=legacy if legacy.startswith("q_") else None,
                    )
                    imported += 1
                queues.pop(sid, None)
            if imported:
                state["queues"] = queues
                features._save_state_unlocked(state)
                STORE.event(kind="runtime.legacy_queue_migrated", data={"items": imported})
            MIGRATION_DONE = True
    except Exception:
        pass
...
def worker(features: Any) -> None:
    ...
    lease = WorkerLease(STORE.paths.root)
    try:
        ...
        while not stop.is_set():
            try:
                ...
                STORE.event(
                    kind="runtime.worker_error",
                    data={"error": f"{type(exc).__name__}: {exc}"[:1000]},
                )
            stop.wait(1.5)
        # A graceful handover must not recover a task while our finalizer still
...
def install(features: Any) -> None:
    ...
    with INSTALL_LOCK:
        if INSTALLED:
            return
        STORE.initialize()
        features.enqueue_prompt = lambda payload: enqueue_prompt(features, payload)
        features.queue_snapshot = queue_snapshot
        features.delete_queue_item = delete_queue_item
        features.reorder_queue = reorder_queue
        features._worker = lambda: worker(features)
        INSTALLED = True


def _read_json(handler: Any, limit: int = 2_000_000) -> dict[str, Any]:
    if hasattr(handler, "_feature_body"):
        return handler._feature_body(limit)
...
def _error(handler: Any, exc: Exception) -> None:
    handler.json_response(
        ...
        status=(
            ...
            else 400 if isinstance(exc, (ValueError, PermissionError)) else 500
        ),
    )
...
def _create_worktree(project_dir: str, task_id: str) -> str:
    ...
    canonical = Path(probe.stdout.strip()).resolve(strict=True)
    target = STORE.paths.worktrees / task_id
    if target.exists():
        raise ValueError("worktree already exists")
    result = subprocess.run(
        ["git", "-C", str(canonical), "worktree", "add", "--detach", str(target), "HEAD"],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
        timeout=30.0,
...
def _remove_worktree(task: dict[str, Any]) -> dict[str, Any]:
    ...
    root = str(metadata.get("ownershipRoot") or "")
    if not worktree or not root:
        raise ValueError("task has no managed worktree")
    ...
    if status.returncode != 0:
        raise RuntimeError(status.stderr.strip() or "worktree status failed")
    if status.stdout.strip():
        raise ValueError("worktree has uncommitted changes; refusing cleanup")
    result = subprocess.run(
    ...
    STORE.update_task(task["id"], metadata_patch={"worktree": None})
    STORE.event(
...
def _cleanup_new_worktree(root: str, worktree: str) -> None:
    ...
    target = Path(worktree).resolve(strict=True)
    managed = STORE.paths.worktrees.resolve(strict=False)
    try:
        target.relative_to(managed)
    except ValueError as exc:
        raise RuntimeError("refusing to clean unmanaged worktree") from exc
    result = subprocess.run(
        ["git", "-C", root, "worktree", "remove", str(target)],
        stdout=subprocess.PIPE,
        stderr=subprocess.PIPE,
        text=True,
        timeout=20.0,
    ...
    if result.returncode != 0:
        raise RuntimeError(result.stderr.strip() or "failed to clean newly-created worktree")


def create_task_request(features: Any, payload: dict[str, Any]) -> dict[str, Any]:
    base = str(payload.get("sessionID") or "")
    directory = (
        features._session_directory(base)
        if base
        else features._canonical_directory(str(payload.get("directory") or ""))
    )
    text = str(payload.get("text") or "").strip()
    if not text:
        raise ValueError("task text is required")
    profile = _profile_id(payload.get("profile") or "build")
    task_id = f"t_{uuid4().hex}"
    isolate = bool(payload.get("isolate"))
    task_dir = directory
    sid = base
    metadata = {
        "ownershipRoot": directory,
        "modeAtCreate": str(payload.get("mode") or "build"),
        "sandbox": REGISTRY.profiles().get(profile, {}).get("sandbox", "repo-write"),
    }
    created_session = False
    if isolate:
        task_dir = _create_worktree(directory, task_id)
        metadata["worktree"] = task_dir
        body = {
        ...
        try:
            ...
            if not isinstance(created, dict) or not created.get("id"):
                raise RuntimeError("OpenCode session creation failed for worktree task")
        except Exception:
    ...
    try:
        if not sid:
            raise ValueError("sessionID is required unless isolate creates a session")
        metadata["usageBaseline"] = _usage_totals(features, sid)
        task = STORE.create_task(
            session_id=sid,
            ...
            kind=str(payload.get("kind") or "prompt")[:80],
            metadata=metadata,
            baseline=git_snapshot(task_dir),
            task_id=task_id,
        )
        STORE.checkpoint(
            task_id,
            "created",
            summary="Standalone server task created"
            + (" in isolated Git worktree" if isolate else ""),
            data={"worktree": metadata.get("worktree"), "ownershipRoot": directory},
        )
        ...
        if created_session:
            try:
                features._backend_request_json(
                    "DELETE", f"/api/session/{quote(sid,safe='')}", None, timeout=12.0
                )
            except Exception:
                pass
        if isolate:
...
def task_control(features: Any, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if action == "pause":
        ...
        if task["state"] in {"running", "submitted", "waiting_permission"}:
            _interrupt(features, task["session_id"])
        task = STORE.transition(task_id, "paused", event="task.paused", data={"source": "user"})
        STORE.checkpoint(
            task_id,
            "paused",
            summary="Task paused; resume continues from durable checkpoint",
            data={"repo": git_snapshot(task["project_dir"])},
        )
    elif action == "resume":
        if task["state"] not in {"paused", "recovering", "needs_attention"}:
        ...
        task = STORE.transition(
            task_id, "cancelled", event="task.cancelled", data={"source": "user"}
        )
    elif action == "priority":
        task = STORE.update_task(task_id, priority=int(payload.get("priority") or 0))
        ...
        return {
            ...
            "checkpoint": STORE.checkpoint(
                task_id,
                str(payload.get("stage") or "manual")[:120],
                summary=str(payload.get("summary") or "")[:4000],
                data=payload.get("data") if isinstance(payload.get("data"), dict) else {},
            ),
        }
    elif action == "cleanup-worktree":
        return {"ok": True, "task": _public(task), "cleanup": _remove_worktree(task)}
    elif action == "handoff":
        handoff = payload.get("handoff") if isinstance(payload.get("handoff"), dict) else {}
        task = STORE.update_task(task_id, metadata_patch={"handoff": handoff})
        STORE.event(
            kind="handoff.updated",
            task_id=task_id,
            session_id=task["session_id"],
            project_dir=task["project_dir"],
            data={"keys": sorted(handoff)},
        )
    else:
...
def spawn_speculative(features: Any, payload: dict[str, Any]) -> dict[str, Any]:
    ...
    if not sid or not text:
        raise ValueError("sessionID and text are required")
    count = max(2, min(3, int(payload.get("count") or 2)))
    directory = features._session_directory(sid)
    children = []
    # Allocate the shared root before children become dispatchable. Forked
    # research and the aggregate must not each receive a new full budget.
    from execution_ledger import ExecutionLedger

    ledger = ExecutionLedger(STORE)
    binding = ledger.binding(sid)
    root_id = binding["root_id"] if binding else f"speculative_{uuid4().hex}"
    for index in range(count):
        fork = _fork(features, sid)
        child_sid = str(fork["id"])
        child = STORE.create_task(
            session_id=child_sid,
            project_dir=directory,
            text=f"Independent researcher {index+1}/{count}. Investigate this problem read-only. Return concrete evidence, risks and a recommended approach.\n\n{text}",
            files=[],
            profile="fast",
            priority=20,
            kind="research",
            metadata={
                "modeAtCreate": "plan",
                "usageBaseline": _usage_totals(features, child_sid),
                "sandbox": "safe",
                "speculativeRootSession": sid,
                "rootTaskID": root_id,
            },
            baseline=git_snapshot(directory),
        )
    ...
    parent = STORE.create_task(
        session_id=sid,
        project_dir=directory,
        text="Compare the independent researcher findings in the structured mailbox. Resolve disagreements, then produce one compact implementation plan or continue the requested work.",
        files=[],
        profile="research",
        priority=30,
        ...
        metadata={
            "modeAtCreate": "plan",
            "usageBaseline": _usage_totals(features, sid),
            "sandbox": "safe",
            "speculative": True,
            "rootTaskID": root_id,
        },
    ...
    for child in children:
        STORE.update_task(child["id"], metadata_patch={"mailboxTo": parent["id"]})
    return {
        "ok": True,
        "parent": _public(STORE.get_task(parent["id"]) or parent),
        "children": [_public(STORE.get_task(item["id"]) or item) for item in children],
    }


def mcp_gateway(features: Any, directory: str, namespace: str | None = None) -> dict[str, Any]:
    key = hashlib.sha256(directory.encode()).hexdigest()
    cached = STORE.cache_get("mcp-health", key)
    if not isinstance(cached, dict):
        try:
            value = features._data(
                features._backend_request_json(
                    "GET", features._workspace_target("/api/mcp", directory), timeout=10.0
                )
            )
        except Exception as exc:
        ...
        if isinstance(value, list):
            for item in value:
                if isinstance(item, dict):
                    rows.append(
                        {
                            "namespace": str(item.get("name") or item.get("id") or ""),
                            "status": (
                                item.get("status")
                                if isinstance(item.get("status"), (str, dict))
                                else "unknown"
                            ),
                        }
                    )
        ...
        STORE.cache_set("mcp-health", key, cached, ttl_seconds=15)
    result = dict(cached)
    if namespace:
        selected = next(
            (item for item in result.get("namespaces", []) if item.get("namespace") == namespace),
            None,
        )
        detail = {
            "namespace": namespace,
            "status": selected.get("status") if selected else "missing",
        }
        if namespace == "kb":
            try:
                probe = features._run_rag_probe("status")
                detail["tools"] = (
                    sorted(str(item) for item in (probe.get("tools") or []))
...
def _parse_plan_document(text: str, fallback_title: str) -> dict[str, Any]:
    ...
    for line in str(text or "").splitlines():
        ...
        if checklist:
            marker = checklist.group(1).lower()
            status = (
                "completed"
                if marker == "x"
                else "in_progress" if marker in {">", "~", "-"} else "pending"
            )
            checked.append({"content": checklist.group(2), "status": status})
            continue
        bullet = re.match(r"^\s*(?:[-*+]|\d+[.)])\s+(.+?)\s*$", line)
        if bullet and not re.match(r"^\[[ xX>~-]\]", bullet.group(1)):
            bullets.append({"content": bullet.group(1), "status": "pending"})
...
def _read_plan_candidate(path: Path, root: Path) -> dict[str, Any] | None:
    try:
        ...
        if not resolved.is_file() or details.st_size > PLAN_MAX_BYTES:
            return None
        ...
        return {
            **parsed,
            "todos": todos[:PLAN_MAX_ITEMS],
            "total": len(todos),
            "completed": completed,
            "inProgress": in_progress,
            "truncated": len(todos) > PLAN_MAX_ITEMS,
            "filename": resolved.name,
            "updated": int(details.st_mtime * 1000),
            "source": "native-v2",
        }
...
def runtime_snapshot(features: Any, directory: str | None = None) -> dict[str, Any]:
    ...
    return {
        ...
        "services": {
            ...
            "typedHandoff": True,
            "verification": True,
            "failureClassifier": True,
            "loopDetector": True,
            "stuckWatchdog": True,
            "patchOwnership": True,
            "speculativeParallelism": True,
            "providerPinnedRoleRouter": True,
            "capabilityRegistry": True,
            "mcpGatewayMetadata": True,
...
def handle_get(handler: Any, parsed: Any, features: Any) -> bool:
    ...
    if parsed.path == "/client-plan.json":
        try:
            plan = (
                session_plan_document(str(params["sessionID"][0]))
                if "sessionID" in params
                else latest_plan_document()
            )
    ...
    try:
        ...
        try:
            directory = (
                features._session_directory(str(sid))
                if sid
                else (params.get("directory") or [None])[0]
            )
            directory = features._canonical_directory(str(directory)) if directory else None
        except ValueError as exc:
            if (
                parsed.path != "/client-tasks.json"
                or not sid
                or str(exc) != "directory outside allowed project roots"
            ):
                raise
            handler.json_response(
                {
                    "ok": True,
                    "available": False,
                    "reason": "project-outside-roots",
                    "tasks": [],
                    "counts": {},
                }
            )
            return True
        if parsed.path == "/client-runtime.json":
            handler.json_response(runtime_snapshot(features, directory))
        elif parsed.path == "/client-tasks.json":
            states = [
                item for item in (params.get("state") or [""])[0].split(",") if item in TASK_STATES
            ]
            ...
            task_id = str((params.get("id") or params.get("taskID") or [""])[0])
            task = STORE.get_task(task_id)
            if not task:
                raise KeyError(task_id)
            handler.json_response(
                {
                    "ok": True,
                    "task": _public(task),
                    "checkpoints": STORE.checkpoints(task_id),
                    "events": STORE.events(task_id=task_id),
                    "artifacts": ARTIFACTS.list(task_id),
                    "usage": STORE.usage_summary(task_id),
                    "mailbox": STORE.mailbox_receive(task_id),
                }
            )
        elif parsed.path == "/client-task-events.json":
            handler.json_response(
                {
                    "ok": True,
                    "events": STORE.events(
                        task_id=(params.get("taskID") or [None])[0],
                        project_dir=directory,
            ...
            if not directory:
                raise ValueError("directory or sessionID is required")
            handler.json_response(capability_snapshot(features, directory))
        elif parsed.path == "/client-resource-status.json":
            handler.json_response(resource_snapshot())
        elif parsed.path == "/client-repo-index.json":
            if not directory:
                raise ValueError("directory or sessionID is required")
            query = str((params.get("q") or [""])[0])
            handler.json_response(
                INDEXER.search(directory, query, limit=int((params.get("limit") or [40])[0]))
                if query
                else INDEXER.refresh(directory)
            )
        elif parsed.path == "/client-artifact.json":
            aid = str((params.get("id") or [""])[0])
            value = ARTIFACTS.get(
                aid,
                offset=int((params.get("offset") or [0])[0]),
                limit=int((params.get("limit") or [64000])[0]),
            ...
            if not directory:
                raise ValueError("directory or sessionID is required")
            handler.json_response(
                mcp_gateway(features, directory, (params.get("namespace") or [None])[0])
            )
        return True
    except Exception as exc:
        _error(handler, exc)
...
def handle_post(handler: Any, parsed: Any, features: Any) -> bool:
    ...
    try:
        ...
        if parsed.path == "/client-task-control.json":
            handler.json_response(task_control(features, payload))
        elif parsed.path == "/client-task-create.json":
            handler.json_response(create_task_request(features, payload))
        elif parsed.path == "/client-project-memory.json":
            directory = (
                features._session_directory(str(payload["sessionID"]))
                if payload.get("sessionID")
                else features._canonical_directory(str(payload.get("directory") or ""))
            )
            STORE.memory_set(
                directory,
                str(payload.get("key") or "note"),
                str(payload.get("value") or ""),
                str(payload.get("category") or "note"),
            )
            handler.json_response({"ok": True, "memory": STORE.memory_list(directory)})
        elif parsed.path == "/client-decisions.json":
            directory = (
            ...
            handler.json_response(
                {
                    ...
                    "decision": STORE.decision_add(
                        directory,
                        str(payload.get("title") or "Decision"),
            ...
            task = STORE.get_task(str(payload.get("toTask") or payload.get("fromTask") or ""))
            directory = str(payload.get("projectDir") or (task or {}).get("project_dir") or "")
            if not directory:
                raise ValueError("projectDir or task reference required")
            item = STORE.mailbox_send(
                project_dir=directory,
                from_task=str(payload.get("fromTask") or "") or None,
                to_task=str(payload.get("toTask") or "") or None,
                message_type=str(payload.get("type") or "finding"),
                payload=(
                    payload.get("payload")
                    if isinstance(payload.get("payload"), dict)
                    else {"text": str(payload.get("text") or "")}
                ),
            )
            handler.json_response({"ok": True, "message": item})
        elif parsed.path == "/client-speculate.json":
            handler.json_response(spawn_speculative(features, payload))
        elif parsed.path == "/client-artifact.json":
            handler.json_response(
                {
                    "ok": True,
                    "artifact": ARTIFACTS.put(
                        ...
                        project_dir=str(payload.get("projectDir") or "") or None,
