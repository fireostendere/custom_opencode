...
SCHEMA_VERSION = 1
TASK_STATES = {
    "queued", "blocked", "paused", "submitted", "running", "waiting_permission",
...
class RuntimeStore:
    def __init__(self, db_path: str | Path | None = None):
        ...
        self.paths = StorePaths(root=root, db=db, artifacts=root / "artifacts", worktrees=root / "worktrees")
    ...
    def create_task(self, *, session_id: str, project_dir: str, text: str = "", files: list[Any] | None = None,
                    profile: str = "direct", priority: int = 0, dependencies: Iterable[str] = (), kind: str = "prompt",
                    metadata: dict[str, Any] | None = None, baseline: dict[str, Any] | None = None,
                    route: dict[str, Any] | None = None, task_id: str | None = None) -> dict[str, Any]:
        self.initialize()
        ...
        with self.transaction() as db:
            db.execute(
                ...
                (task_id,session_id,project_dir,kind,profile,priority,"blocked" if deps else "queued",text,_json(files or []),
                 _json(metadata or {}),timestamp,timestamp,timestamp,_json(baseline or {}),_json(route or {})),
            )
            for dependency in dict.fromkeys(deps):
                db.execute("INSERT OR IGNORE INTO task_dependencies(task_id,depends_on) VALUES(?,?)", (task_id,dependency))
            self._event_db(db,task_id,session_id,project_dir,"task.created",{"priority":priority,"profile":profile,"dependencies":deps,"kind":kind},timestamp)
    ...
    def list_tasks(self, *, session_id: str | None = None, project_dir: str | None = None,
                   states: Iterable[str] | None = None, limit: int = 200) -> list[dict[str, Any]]:
        self.initialize(); clauses: list[str] = []; args: list[Any] = []
        ...
        if state_list: clauses.append("state IN (%s)" % ",".join("?" for _ in state_list)); args.extend(state_list)
    ...
    def dependencies(self, task_id: str) -> list[str]:
        self.initialize()
        with self.connect() as db:
            rows = db.execute("SELECT depends_on FROM task_dependencies WHERE task_id=? ORDER BY depends_on", (task_id,)).fetchall()
    ...
    def dependency_state(self, task_id: str) -> tuple[bool,list[str]]:
        ...
        with self.connect() as db:
            rows = db.execute("SELECT id,state FROM tasks WHERE id IN (%s)" % ",".join("?" for _ in deps), deps).fetchall()
    ...
    def transition(self, task_id: str, state: str, *, event: str | None = None,
        ...
        with self.transaction() as db:
            ...
            updates=["state=?","updated_at=?","last_progress_at=?"]; args:list[Any]=[state,timestamp,timestamp]
            if state in {"submitted","running"} and current["started_at"] is None: updates.append("started_at=?"); args.append(timestamp)
    ...
    def update_task(self, task_id: str, *, priority: int | None = None, dependencies: Iterable[str] | None = None,
                    metadata_patch: dict[str, Any] | None = None, route: dict[str, Any] | None = None,
    ...
    def reorder(self, session_id: str, ids: list[str]) -> None:
        for index,task_id in enumerate(ids[:200]):
            ...
            if task and task["session_id"]==session_id and task["state"] in {"queued","blocked","paused"}: self.update_task(task_id,priority=max(-100,50-index))

    def event(self, *, kind: str, task_id: str | None = None, session_id: str | None = None,
              project_dir: str | None = None, data: dict[str, Any] | None = None) -> int:
        self.initialize(); timestamp=now_ms()
        with self.transaction() as db: return self._event_db(db,task_id,session_id,project_dir,kind,data or {},timestamp)

    def _event_db(self, db: sqlite3.Connection, task_id: str | None, session_id: str | None,
    ...
    def checkpoint(self, task_id: str, stage: str, *, summary: str = "", data: dict[str, Any] | None = None) -> dict[str, Any]:
        task=self.get_task(task_id)
        if not task: raise KeyError(task_id)
        cid=f"cp_{uuid4().hex}"; timestamp=now_ms()
        with self.transaction() as db:
            db.execute("INSERT INTO checkpoints(id,task_id,stage,summary,data_json,created_at) VALUES(?,?,?,?,?,?)",(cid,task_id,stage,str(summary)[:4000],_json(data or {}),timestamp))
            self._event_db(db,task_id,task["session_id"],task["project_dir"],"checkpoint.saved",{"checkpointID":cid,"stage":stage,"summary":str(summary)[:500]},timestamp)
        return {"id":cid,"taskID":task_id,"stage":stage,"summary":str(summary)[:4000],"data":data or {},"createdAt":timestamp}

    def checkpoints(self, task_id: str, limit: int = 100) -> list[dict[str, Any]]:
        self.initialize()
        with self.connect() as db: rows=db.execute("SELECT * FROM checkpoints WHERE task_id=? ORDER BY created_at DESC LIMIT ?",(task_id,max(1,min(500,int(limit))))).fetchall()
        return [{"id":r["id"],"taskID":r["task_id"],"stage":r["stage"],"summary":r["summary"],"data":_loads(r["data_json"],{}),"createdAt":r["created_at"]} for r in rows]

    def cache_set(self, namespace: str, key: str, value: Any, ttl_seconds: float | None = None) -> None:
        self.initialize(); timestamp=now_ms(); expires=timestamp+int(ttl_seconds*1000) if ttl_seconds else None
        with self.transaction() as db: db.execute("INSERT OR REPLACE INTO cache(namespace,cache_key,value_json,expires_at,updated_at) VALUES(?,?,?,?,?)",(namespace,key,_json(value),expires,timestamp))

    def cache_get(self, namespace: str, key: str) -> Any | None:
        self.initialize(); timestamp=now_ms()
        with self.connect() as db: row=db.execute("SELECT value_json,expires_at FROM cache WHERE namespace=? AND cache_key=?",(namespace,key)).fetchone()
        if not row: return None
        if row["expires_at"] is not None and int(row["expires_at"])<timestamp:
            with self.transaction() as db: db.execute("DELETE FROM cache WHERE namespace=? AND cache_key=?",(namespace,key))
            return None
    ...
    def add_usage(self, *, task_id: str | None, model_ref: str | None, stage: str, input_tokens: int = 0,
        ...
        self.initialize()
        with self.transaction() as db: db.execute("INSERT INTO usage(task_id,model_ref,stage,input_tokens,output_tokens,cache_read_tokens,cache_write_tokens,cost,latency_ms,success,created_at) VALUES(?,?,?,?,?,?,?,?,?,?,?)",(task_id,model_ref,stage,int(input_tokens),int(output_tokens),int(cache_read_tokens),int(cache_write_tokens),float(cost),int(latency_ms),None if success is None else int(bool(success)),now_ms()))

    def usage_summary(self, task_id: str | None = None) -> dict[str, Any]:
        self.initialize(); where=" WHERE task_id=?" if task_id else ""; args=(task_id,) if task_id else ()
        with self.connect() as db: rows=db.execute(f"SELECT stage,SUM(input_tokens) i,SUM(output_tokens) o,SUM(cache_read_tokens) cr,SUM(cache_write_tokens) cw,SUM(cost) cost,SUM(latency_ms) latency,COUNT(*) n FROM usage{where} GROUP BY stage",args).fetchall()
        stages={str(r["stage"]):{"inputTokens":int(r["i"] or 0),"outputTokens":int(r["o"] or 0),"cacheReadTokens":int(r["cr"] or 0),"cacheWriteTokens":int(r["cw"] or 0),"cost":float(r["cost"] or 0),"latencyMs":int(r["latency"] or 0),"samples":int(r["n"] or 0)} for r in rows}
        return {"stages":stages,"total":{"inputTokens":sum(v["inputTokens"] for v in stages.values()),"outputTokens":sum(v["outputTokens"] for v in stages.values()),"cost":sum(v["cost"] for v in stages.values()),"latencyMs":sum(v["latencyMs"] for v in stages.values())}}

    def model_stats(self, model_ref: str, limit: int = 200) -> dict[str, Any]:
        self.initialize()
        with self.connect() as db: rows=db.execute("SELECT success,latency_ms,cost FROM usage WHERE model_ref=? ORDER BY id DESC LIMIT ?",(model_ref,max(1,min(1000,int(limit))))).fetchall()
        if not rows: return {"samples":0,"successRate":None,"avgLatencyMs":None,"avgCost":None}
        known=[int(r["success"]) for r in rows if r["success"] is not None]
        return {"samples":len(rows),"successRate":sum(known)/len(known) if known else None,"avgLatencyMs":sum(int(r["latency_ms"] or 0) for r in rows)/len(rows),"avgCost":sum(float(r["cost"] or 0) for r in rows)/len(rows)}

    def memory_set(self, project_dir: str, key: str, value: str, category: str = "note") -> None:
        self.initialize()
        with self.transaction() as db: db.execute("INSERT OR REPLACE INTO project_memory(project_dir,memory_key,value,category,updated_at) VALUES(?,?,?,?,?)",(project_dir,key[:240],value[:20000],category[:80],now_ms()))

    def memory_list(self, project_dir: str, limit: int = 100) -> list[dict[str, Any]]:
        self.initialize()
        with self.connect() as db: rows=db.execute("SELECT memory_key,value,category,updated_at FROM project_memory WHERE project_dir=? ORDER BY updated_at DESC LIMIT ?",(project_dir,max(1,min(500,int(limit))))).fetchall()
        return [{"key":r["memory_key"],"value":r["value"],"category":r["category"],"updatedAt":r["updated_at"]} for r in rows]

    def decision_add(self, project_dir: str, title: str, decision: str, rationale: str = "") -> dict[str, Any]:
        self.initialize(); item=f"d_{uuid4().hex}"; timestamp=now_ms()
        with self.transaction() as db: db.execute("INSERT INTO decisions(id,project_dir,title,decision,rationale,status,created_at,updated_at) VALUES(?,?,?,?,?,?,?,?)",(item,project_dir,title[:500],decision[:12000],rationale[:12000],"active",timestamp,timestamp))
        return {"id":item,"projectDir":project_dir,"title":title[:500],"decision":decision[:12000],"rationale":rationale[:12000],"status":"active","createdAt":timestamp}

    def decision_list(self, project_dir: str, limit: int = 100) -> list[dict[str, Any]]:
        self.initialize()
        with self.connect() as db: rows=db.execute("SELECT * FROM decisions WHERE project_dir=? AND status='active' ORDER BY updated_at DESC LIMIT ?",(project_dir,max(1,min(500,int(limit))))).fetchall()
        return [{"id":r["id"],"title":r["title"],"decision":r["decision"],"rationale":r["rationale"],"status":r["status"],"createdAt":r["created_at"],"updatedAt":r["updated_at"]} for r in rows]

    def mailbox_send(self, *, project_dir: str, message_type: str, payload: dict[str, Any], from_task: str | None = None, to_task: str | None = None) -> dict[str, Any]:
        self.initialize(); item=f"m_{uuid4().hex}"; timestamp=now_ms()
        with self.transaction() as db: db.execute("INSERT INTO mailbox(id,project_dir,from_task,to_task,message_type,payload_json,consumed,created_at) VALUES(?,?,?,?,?,?,0,?)",(item,project_dir,from_task,to_task,message_type[:80],_json(payload),timestamp))
        return {"id":item,"projectDir":project_dir,"fromTask":from_task,"toTask":to_task,"type":message_type[:80],"payload":payload,"createdAt":timestamp}

    def mailbox_receive(self, task_id: str, *, consume: bool = False, limit: int = 100) -> list[dict[str, Any]]:
        self.initialize()
        with self.transaction() as db:
            rows=db.execute("SELECT * FROM mailbox WHERE to_task=? AND consumed=0 ORDER BY created_at ASC LIMIT ?",(task_id,max(1,min(500,int(limit))))).fetchall(); ids=[str(r["id"]) for r in rows]
            if consume and ids: db.execute("UPDATE mailbox SET consumed=1 WHERE id IN (%s)" % ",".join("?" for _ in ids),ids)
    ...
    def recover_inflight(self) -> int:
        self.initialize(); timestamp=now_ms()
        with self.transaction() as db:
            rows=db.execute("SELECT id,session_id,project_dir,state FROM tasks WHERE state IN ('submitted','running','verifying','waiting_permission')").fetchall()
            for r in rows:
                db.execute("UPDATE tasks SET state='recovering',updated_at=?,last_progress_at=? WHERE id=?",(timestamp,timestamp,r["id"]))
                self._event_db(db,r["id"],r["session_id"],r["project_dir"],"task.recovering",{"previousState":r["state"]},timestamp)
        return len(rows)
